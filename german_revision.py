...
ARABIC = "العربية"
GERMAN = "الألمانية"
# CSV files are kept beside this app so they can be uploaded directly into
# the ``german`` folder on GitHub without needing a nested directory.
LIST_DIRECTORY = Path(__file__).parent
...
tab_words, tab_quiz = st.tabs(["الكلمات والاستماع", "اختبر نفسك"])

with tab_words:
    st.subheader(f"استمع إلى المجموعة {batch_number}")
    st.caption("تظهر الكلمات في الجدول أسفل المشغّل أثناء النطق الألماني المحلي.")
    german_speech_player(
        ". ".join(batch[GERMAN].tolist()),
        key=f"speech_{selected_list}_{batch_number}",
    )
